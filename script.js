...
class Book {
  ...
  //Basically "Master" method. Gets called when the "Add Book" Button is pressed. Creates a new entry in the books list with the input Title, Author, Pagesnumber and Read status. Also calls functions to add Button functionality for both buttons (Read and delete)
  addBook() {
...
function addBookDiv() {
  library.forEach(function (book, index) {
    //for each book in the array creates a book div and adds it
    ...
    if (!book.read) {
      //if book has not been read then change style
...
function activateRemoveButtons() {
  ...
  deleteArray.forEach(function (button) {
    button.addEventListener("click", () => {
      //For every book add EVL to remove the element form the DOM.
